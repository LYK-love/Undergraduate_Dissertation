...
from Project.code.utils.label_correction_utils import Lemmatizer, tokenize_subtokenize_comment, tokenize_clean_code
...
def positive_data_clean(input_file_dir_path, output_file_dir_path):

    examples = []
    positive_sample_id = []
    for file in ['train', 'valid', 'test']:
        with open(input_file_dir_path / f'{file}.jsonl', 'r') as f:
            for line in f:
                obj = json.loads(line)
    ...
    with open( output_file_dir_path / 'positive_sample_id.json', 'w') as f:  # used to select reliable samples
    ...
    for example in examples:
        ...
        if not flag:
            ...
            continue
    with open( output_file_dir_path / 'positive_id.json', 'w') as f:
        json.dump(positive_id, f)
    print(f"LABEL CORRECTION END! Save file in dir {output_file_dir_path}")


def label_correction(original_update_dataset_dir='/root/projects/TestData/data/cup2_updater_dataset_test', output_file_dir_path='/root/projects/TestData/data_process'):
    positive_data_clean(Path(original_update_dataset_dir), Path(output_file_dir_path))



if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--input_file_dir_path', required=True, type=str, help='path to the dir of original updater data')
    parser.add_argument('--output_file_dir_path', default='/root/projects/TestData/data_process', type=str, help='path to thr dir of output data, by default it will be the current dir. And the output files are "positive_id.json" and "positive_sample_id.json" ')


    args = parser.parse_args()
    input_file_dir_path = Path(args.input_file_dir_path)
    output_file_dir_path = Path(args.output_file_dir_path)
    positive_data_clean(input_file_dir_path, output_file_dir_path)
